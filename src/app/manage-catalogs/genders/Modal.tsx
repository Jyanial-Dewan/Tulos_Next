...
const Modal = ({
  ...
  const GenderSchema = z.object({
    gender_name: z.string().min(1, "Gender name is required"),
  });
  ...
  const form = useForm<genderForm>({
    ...
    defaultValues: {
      gender_name: action === "edit" ? selectedItems[0].gender_name : "",
    },
  });
    ...
    if (action === "edit" && selectedItems[0]) {
      const fetchGender = async () => {
        const res = await loadData({
          url: `${endpoints.Genders}?gender_id=${selectedItems[0].gender_id}`,
          setLoading: setIsLoading,
          //   accessToken: token.access_token,
        });

        form.reset({
          gender_name: res?.data.result.gender_name,
        });
      };
  ...
  const onSubmit = async (data: genderForm) => {
    const payload = {
      ...data,
    };
    if (action === "add") {
  ...
  return (
    <>
      {action && openModal && catalogType === "gender" && (
